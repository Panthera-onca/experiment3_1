...
from twittervotes.core.app_logger import get_logger
...
def validated_args(args):
    ...
    if len(unique_hashtags) < len(args.hashtags):
        logger.info('Some hashtags passed as arguments were duplicated and are going to be ignored')
...
def parse_commandline_args() -> object:
    ...
    required.add_argument('-ht', '--hashtags',
                          ...
                          required=True,
                          dest='hashtags',
                          help=('Space-separated list specifying the hashtags that will be used for the voting.\n'
                                'Type the hashtags without a hash symbol.'))
